...
namespace API.Application.ModuloEventos
{
    public class EventosApp : BaseApp<EventosApp>
    {
        private IEventosRepository _eventoRepository;
        public EventosApp(IEventosRepository _EventoRepository, ILogger<BaseApp<EventosApp>> logger
                                   ) : base(logger)
        {
        ...
        public async Task<StatusReponse<List<ListarEventosAdmin>>> AdminList(int perfil, int usuario)
        {
            StatusReponse<List<ListarEventosAdmin>> status = null;
            status = await this.ComplexResponse(() => _eventoRepository.AdminListarEventos(perfil, usuario));
            return status;
        }
